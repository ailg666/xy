...
if command -v crontab >/dev/null 2>&1; then
	...
	echo "$minu $hour */$4 * * screen -dmS emby_sync bash -c \"\$(curl https://xy.ggbond.org/xy/sync_emby_config_ailg.sh) -s $1 $2 $5 | tee $1/temp/cron.log" >> /tmp/cronjob.tmp
	...
	echo "$minu $hour */$4 * * root screen -dmS emby_sync bash -c \"\$(curl https://xy.ggbond.org/xy/sync_emby_config_ailg.sh) -s $1 $2 $5 | tee $1/temp/cron.log" >> /etc/crontab
    echo -e "\n"
    echo -e "———————————————————————————————————— \033[1;33mA  I  老  G\033[0m —————————————————————————————————"
    echo -e "\n"	
    INFO "已经添加下面的记录到crontab定时任务，每$4天更新一次config"
