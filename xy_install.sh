...
user_selecto() {
    while :; do
        ...
        echo -e "\n"
        echo -e "\033[1;32m8、G-Box安装常用镜像下载（注意：该功能暂不可用，新方案正在测试中）\033[0m\033[0m"
        echo -e "\n"
        echo -e "——————————————————————————————————————————————————————————————————————————————————"
...
fix_docker() {
    ...
    clear
    read -p $'\033[1;33m请确保您当前设备与网络没有开启其他代理（即无需梯子魔法，如有请先关闭），否则可能无法正常使用？（y=继续/n=退出）: \033[0m' user_warning
    if [[ "$user_warning" == [Nn] ]]; then
        exit 0
    fi
