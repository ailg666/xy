...
get_emby_status() {
    ...
    while read -r container_id; do
        if docker inspect --format '{{ range .Mounts }}{{ println .Source .Destination }}{{ end }}' $container_id | grep -qE "/xiaoya\b /media\b|\.img /media\.img"; then
...
update_ggbond() {
    ...
    if [ ! "$local_sha" == "$remote_sha" ]; then
        ...
        while [ $retries -lt $max_retries ]; do
            if docker pull "ailg/ggbond:latest"; then
                INFO "ailg/ggbond:latest 镜像拉取成功！"
                break
            else
                WARN "ailg/ggbond:latest 镜像拉取失败，正在进行第 $((retries + 1)) 次重试..."
                retries=$((retries + 1))
            fi
        done
...
function user_select4() {
    ...
    while :; do
        ...
        read -erp "请输入您的选择（1-2，按b返回上级菜单或按q退出）；" f4_select
        case "$f4_select" in
        1)
    ...
    for name in "${!emby_list[@]}"; do
        if [[ "${name}" == "emby" ]]; then
            read -erp "$(echo -e '\033[1;36m是否保留名为emby的容器！按Y/y保留，按其他任意键将删除！\033[0m\n请输入：') " del_emby
    ...
    if [[ ! $answer =~ ^[Nn]$ || -z "$answer" ]]; then
        ...
        if docker ps -a | grep -qE ' xiaoya-emd\b' && docker stop xiaoya-emd && docker rm xiaoya-emd; then
            INFO "${Yellow}已删除您原来的xiaoya-emd容器！${NC}"
        fi
        if docker run -d \
            --name=xiaoya-emd \
            --privileged \
        ...
        if [ ${#img_order[@]} -ne 0 ]; then
            ...
            while :; do
                ...
                if [ "${img_select}" -gt 0 ] && [ "${img_select}" -le ${#img_order[@]} ]; then
                    ...
                    if mount /dev/loop7 ${img_mount}; then
