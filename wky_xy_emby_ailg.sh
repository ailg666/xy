...
function get_config_path(){
    ...
    if [[ $f12_select_0 == [Nn] ]]; then
		echo -e "\033[1;35m请输入您的小雅ALIST老G版配置文件路径:\033[0m"
        read config_dir
        if ! [[ -d "$config_dir" && -f "$config_dir/mytoken.txt" ]]; then
            ERROR "该路径不存在或该路径下没有mytoken.txt配置文件"
			ERROR "如果你是选择全新目录重装小雅ALIST老G版，请先删除原来的容器，再重新运行本脚本！"
...
function user_select1(){
    ...
    echo -e "\n"
    echo -e "\033[1;35m2、卸载小雅ALIST老G版\033[0m"
    echo -e "\n"
    echo -e "——————————————————————————————————————————————————————————————————————————————————"
	...
	if [[ $u1_select == 1 ]];then
		if [[ $st_alist =~ "已安装" ]];then
			...
			check_env
			read -ep "请输入小雅ALIST老G版的安装路径，使用默认的/etc/xiaoya可直接回车：" config_dir
		...
		INFO "${Green}哇塞！你的小雅ALIST老G版安装完成了！$NC"
	...
	if [[ $u2_select == 1 ]];then
		...
		while true; do
			...
			if [ "$elapsed_time" -gt 300 ]; then
				echo "小雅ALIST老G版未正常启动超时 5分钟，请检查小雅ALIST老G版的安装！"
				break
			fi	
			sleep 3
		done
...
function user_select3(){
	echo -e "———————————————————————————————————— \033[1;33mA  I  老  G\033[0m —————————————————————————————————"
    echo -e "\n"
    echo -e "\033[1;32m1、一键安装/重装小雅ALIST老G版+EMBY\033[0m"
    echo -e "\n"
    echo -e "\033[1;35m2、卸载小雅ALIST老G版+EMBY\033[0m"
    echo -e "\n"
    echo -e "——————————————————————————————————————————————————————————————————————————————————"
	...
	if [[ $u3_select == 1 ]];then
		...
		while true; do
			...
			elapsed_time=$((current_time - start_time))
			if [ "$elapsed_time" -gt 300 ]; then
				echo "小雅ALIST老G版未正常启动超时 5分钟，程序将退出，请检查小雅ALIST老G版的安装，或重启小雅ALIST老G版后重新运行脚本！"
				exit
			fi	
			sleep 3
		done
...
function main(){
	...
	echo -e "\e[0m"
	echo -e "小雅ALIST老G版：${st_alist};          小雅emby：${st_emby}"
	echo -e "\e[0m"
	echo -e "———————————————————————————————————— \033[1;33mA  I  老  G\033[0m —————————————————————————————————"
    echo -e "\n"
    echo -e "\033[1;32m0、一键安装casaos\033[0m"
    echo -e "\n"
	echo -e "\033[1;32m1、安装/重装/卸载小雅ALIST老G版\033[0m"
    echo -e "\n"
